...
- (instancetype)initWithDataArray:(NSArray *)dataArray
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)]) {
        ...
        CGFloat H = height*_dataArray.count;
        CGFloat h =SCREEN_HEIGHT-origin.y-10;
        if (H>h)H = h;
        if (direction == kLrdOutputViewDirectionLeft) {
            self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, origin.y, width,H) style:UITableViewStylePlain];
        }else {
            self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, origin.y, -width,H) style:UITableViewStylePlain];
        }
